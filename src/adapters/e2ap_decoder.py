"""E2AP / E2SM-KPM / E2SM-RC Decoder and Encoder for TN-NTN xApp.

Decodes APER-encoded E2AP RICindication messages containing E2SM-KPM v3
measurement data with extended NTN fields (multi-orbit RSRP, elevation,
Doppler), and encodes E2SM-RC control messages for beam handover.

Supports E2SM-TN service model for satellite-specific KPMs.
...
try:
    import mdclogpy
    logger = mdclogpy.Logger(name="tn-ntn-xapp.e2ap")
except ImportError:
...
# E2SM-KPM measurement names (3GPP TS 28.552 + NTN extensions)
_KPM_MEAS_NAMES = {
    ...
    "NTN.ElevationAngle": "elevationDeg",
    "NTN.DopplerShift": "dopplerHz",
    "NTN.PropagationDelay": "delayUs",
    "NTN.RainAttenuation": "rainAttenuationDb",
    "NTN.OrbitType": "orbit_type_encoded",
}
...
_RANP_TARGET_ORBIT = 4


class E2APDecoder:
    """Decodes E2AP/E2SM-KPM indications and encodes E2SM-RC control requests.

    Supports extended NTN fields for multi-orbit broadband handover.
    """
    ...
    def __init__(
        ...
        for path, name, attr in [
            (kpm_schema_path, "E2SM-KPM v3", "_kpm_codec"),
            (rc_schema_path, "E2SM-RC v1", "_rc_codec"),
            (e2ap_schema_path, "E2AP v2", "_e2ap_codec"),
        ]:
            if os.path.exists(path):
                try:
                    codec = asn1tools.compile_files(path, "per")
                    setattr(self, attr, codec)
                    logger.info("%s schema compiled", name)
                except Exception as e:
                    logger.warning("%s schema compilation failed: %s", name, e)
            else:
                logger.warning("%s schema not found: %s", name, path)
    ...
    def decode_kpm_indication_message(self, message_bytes: bytes) -> dict:
        """Decode E2SM-KPM IndicationMessage with NTN extensions."""
        if not self._kpm_codec:
    ...
    def _parse_kpm_format1(self, fmt1: dict) -> dict:
        """Parse E2SM-KPM IndicationMessage-Format1."""
            ...
            for info_item in fmt1["measInfoList"]:
                ...
                if isinstance(meas_type, tuple):
                    if meas_type[0] == "measName":
                        name = meas_type[1]
                        meas_names.append(_KPM_MEAS_NAMES.get(name, name))
    ...
    def _parse_kpm_format3(self, fmt3: dict) -> dict:
        """Parse E2SM-KPM IndicationMessage-Format3 (per-UE reports)."""
    ...
    def decode_ric_indication(self, payload: bytes) -> dict:
        ...
        # Direct E2SM-KPM decode (OSC RIC style)
    ...
    def _extract_from_e2ap_indication(self, e2ap_decoded: dict) -> dict:
        """Extract E2SM payloads from decoded E2AP RICindication."""
    ...
    def encode_ric_control(self, cell_id: str, action: dict) -> Optional[bytes]:
        """Encode E2SM-RC control request for beam handover.

        Supports target orbit specification for multi-orbit handover.
        """
        ...
        target_orbit = action.get("targetOrbit", "LEO")
            ...
            message = (
                "controlMessage-Format1",
                {
                    "ranP-List": [
                        ...
                        {
                            "ranParameter-ID": _RANP_TARGET_ORBIT,
                            "ranParameter-valueType": (
                                "ranP-Choice-ElementTrue",
                                {
                                    "ranParameter-value": (
                                        "valueOctS",
                                        target_orbit.encode("utf-8"),
                                    ),
                                },
                            ),
                        },
    ...
    # -- E2 Subscription lifecycle encoding/decoding ----------------------
    ...
    def encode_subscription_request(
        ...
        try:
            ...
            ies = [
                {"id": _IE_RIC_REQUEST_ID, "criticality": "reject", "value": (
                    "ricRequestID", {"ricRequestorID": requestor_id, "ricInstanceID": instance_id},
                )},
                {"id": _IE_RAN_FUNCTION_ID, "criticality": "reject", "value": (
                    "ranFunctionID", ran_function_id,
                )},
                {"id": _IE_RIC_SUBSCRIPTION_DETAILS, "criticality": "reject", "value": (
            ...
            encoded = self._e2ap_codec.encode("RICsubscriptionRequest", {"protocolIEs": ies})
            logger.info(
                "Encoded RICsubscriptionRequest (requestor=%d, instance=%d, ran_func=%d, actions=%d)",
                requestor_id, instance_id, ran_function_id, len(actions),
            )
            return encoded
    ...
    def decode_subscription_response(self, payload: bytes) -> dict:
        """Decode a RICsubscriptionResponse message."""
        if not self._e2ap_codec:
    ...
    def decode_subscription_failure(self, payload: bytes) -> dict:
        """Decode a RICsubscriptionFailure message."""
        if not self._e2ap_codec:
            return {"raw_hex": payload.hex(), "decode_error": "E2AP codec not loaded"}
        try:
